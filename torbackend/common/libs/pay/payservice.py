...
from models.food import FoodSaleChangeLog
...
class PayService():
    ...
    def closeOrder(self,pay_order_id = 0):
        if pay_order_id < 1:
            return False
        pay_order_info = session.query(PayOrder).filter_by(id =  pay_order_id ,status = -8).first()
        if not pay_order_info:
            return False

        pay_order_items = session.query(PayOrderItem).filter_by(pay_order_id = pay_order_id).all()
        if pay_order_items:
            #需要归还库存
            for item in pay_order_items:
                tmp_food_info = session.query(Food).filter_by(id = item.food_id).first()
                if tmp_food_info:
    ...
    def orderSuccess(self,pay_order_id = 0,params = None):
        try:
            pay_order_info = session.query(PayOrder).filter_by(id = pay_order_id).first()
    ...
    def geneOrderSn(self):
        ...
        while True:
            ...
            sn = m.hexdigest()
            if not session.query(PayOrder).filter_by(order_sn = sn).first():
                break
        return sn
